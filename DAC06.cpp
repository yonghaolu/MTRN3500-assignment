...
void EmbeddedDevice::DAC06::analogOutputRaw(uint8_t channel, uint16_t value){
    ...
    eops -> outb(RawValue&0x00FF,base + (2 * channel) );// output to low 
    eops -> outb((RawValue >> 8) & 0x0f ,base + (2 * channel + 1) );//output to high
...
void EmbeddedDevice::DAC06::analogOutputVoltage(uint8_t channel, double desired_voltage){
    uint16_t output = static_cast<uint16_t>((desired_voltage + .5* 10)/10 * 4095);   //range is -5v to 5v
